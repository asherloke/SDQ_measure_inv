...
data_w5_parent <- data_w5 %>% 
  select(MCSID, P_w5_SDQ_01:P_w5_SDQ_25, ethnicity, sex_final, ASD_final) %>% 
  mutate(report = "parent") %>%
  pivot_longer(c(P_w5_SDQ_01:P_w5_SDQ_25), names_to = "item", values_to = "score") %>%
  select(MCSID, item, score, report, ethnicity, sex_final, ASD_final)
```
...
data_w5_parent_positive <- data_w5_parent %>%
...
data_w5_parent_reverse <- data_w5_parent %>%
  filter(item == "P_w5_SDQ_07" | 
           item == "P_w5_SDQ_11" | 
           item == "P_w5_SDQ_14" | 
           item == "P_w5_SDQ_21" | 
...
data_w5_teacher <- data_w5 %>% 
  select(MCSID, T_w5_SDQ_01:T_w5_SDQ_25, ethnicity, sex_final, ASD_final) %>% 
  mutate(report = "teacher") %>%
  pivot_longer(c(T_w5_SDQ_01:T_w5_SDQ_25), names_to = "item", values_to = "score") %>%
  select(MCSID, item, score, report, ethnicity, sex_final, ASD_final)
```
...
#reverse score items 7, 11, 14, 21, 25
data_w5_teacher_positive <- data_w5_teacher %>%
  filter(item != "T_w5_SDQ_07" & 
           item != "T_w5_SDQ_11" & 
           item != "T_w5_SDQ_14" & 
           item != "T_w5_SDQ_21" & 
           item != "T_w5_SDQ_25"
...
data_w5_teacher_reverse <- data_w5_teacher %>%
  filter(item == "T_w5_SDQ_07" | 
           item == "T_w5_SDQ_11" | 
           item == "T_w5_SDQ_14" | 
           item == "T_w5_SDQ_21" | 
...
data_w5_teacher_reverse %>% count(score)

#now make 0, 1, 2, into 2, 1, 0.
data_w5_teacher_reverse <- data_w5_teacher_reverse %>%
  mutate(score = recode(score,
                        "0" = 2,
                        "1" = 1,
                        "2" = 0))
...
#join back
data_w5_teacher <- bind_rows(data_w5_teacher_positive, data_w5_teacher_reverse)

rm(data_w5_teacher_positive)
rm(data_w5_teacher_reverse)
...
data_w5_teacher <- data_w5_teacher %>% 
  pivot_wider(names_from = item, values_from = score)
w5_parent_teacher <- inner_join(data_w5_parent, data_w5_teacher, by = "MCSID")

#No NAs
w5_parent_teacher <- w5_parent_teacher %>% 
  drop_na(P_w5_SDQ_01:P_w5_SDQ_25, T_w5_SDQ_01:T_w5_SDQ_25)

#Recode item names
w5_parent_teacher1 <- w5_parent_teacher %>%
  select(MCSID, report.x, P_w5_SDQ_01:P_w5_SDQ_25, ethnicity.x, sex_final.x, ASD_final.x) %>%
  mutate(Q1 = P_w5_SDQ_01,
         ...
         sex_final = sex_final.x,
         ASD_final = ASD_final.x) %>%
  pivot_longer(Q1:Q25, names_to = "item", values_to = "score") %>%
  select(MCSID, report, item, score, ethnicity, sex_final, ASD_final)

w5_parent_teacher2 <- w5_parent_teacher %>%
  select(MCSID, report.y, T_w5_SDQ_01:T_w5_SDQ_25, ethnicity.y, sex_final.y, ASD_final.y) %>%
  mutate(Q1 = T_w5_SDQ_01,
         ...
         sex_final = sex_final.y,
         ASD_final = ASD_final.y) %>%
  pivot_longer(Q1:Q25, names_to = "item", values_to = "score") %>%
  select(MCSID, report, item, score, ethnicity, sex_final, ASD_final)

data_w5_final <- bind_rows(w5_parent_teacher1, w5_parent_teacher2)

rm(w5_parent_teacher1)
rm(w5_parent_teacher2)
```
...
data_w7_parent <- data_w7 %>% 
  select(MCSID, P_w7_SDQ_01:P_w7_SDQ_25, ethnicity, sex_final, ASD_final) %>% 
  mutate(report = "parent") %>%
  pivot_longer(c(P_w7_SDQ_01:P_w7_SDQ_25), names_to = "item", values_to = "score") %>%
  select(MCSID, item, score, report,ethnicity, sex_final, ASD_final)
```
...
#reverse score items 7, 11, 14, 21, 25
data_w7_parent_positive <- data_w7_parent %>%
  filter(item != "P_w7_SDQ_07" & 
           item != "P_w7_SDQ_11" & 
           item != "P_w7_SDQ_14" & 
           item != "P_w7_SDQ_21" & 
           item != "P_w7_SDQ_25"
...
data_w7_parent_reverse <- data_w7_parent %>%
  filter(item == "P_w7_SDQ_07" | 
           item == "P_w7_SDQ_11" | 
           item == "P_w7_SDQ_14" | 
           item == "P_w7_SDQ_21" | 
...
data_w7_parent_reverse %>% count(score)

#now make 0, 1, 2, into 2, 1, 0.
data_w7_parent_reverse <- data_w7_parent_reverse %>%
  mutate(score = recode(score,
                        "0" = 2,
                        "1" = 1,
                        "2" = 0))
...
#join back
data_w7_parent <- bind_rows(data_w7_parent_positive, data_w7_parent_reverse)

rm(data_w7_parent_positive)
rm(data_w7_parent_reverse)
...
#self-report
data_w7_CM <- data_w7 %>% 
  select(MCSID, CM_w7_SDQ_01:CM_w7_SDQ_25, ethnicity, sex_final, ASD_final) %>% 
  mutate(report = "self") %>%
  pivot_longer(c(CM_w7_SDQ_01:CM_w7_SDQ_25), names_to = "item", values_to = "score") %>%
  select(MCSID, item, score, report, ethnicity, sex_final, ASD_final)
```
...
#reverse score items 7, 11, 14, 21, 25
data_w7_CM_positive <- data_w7_CM %>%
  filter(item != "CM_w7_SDQ_07" & 
           item != "CM_w7_SDQ_11" & 
           item != "CM_w7_SDQ_14" & 
           item != "CM_w7_SDQ_21" & 
           item != "CM_w7_SDQ_25"
...
data_w7_CM_reverse %>% count(score)

#now make 0, 1, 2, into 2, 1, 0.
data_w7_CM_reverse <- data_w7_CM_reverse %>%
  mutate(score = recode(score,
                        "0" = 2,
                        "1" = 1,
                        "2" = 0))
...
#join back
data_w7_CM <- bind_rows(data_w7_CM_positive, data_w7_CM_reverse)

rm(data_w7_CM_positive)
rm(data_w7_CM_reverse)
...
data_w7_CM <- data_w7_CM %>% 
  pivot_wider(names_from = item, values_from = score)
w7_parent_CM <- inner_join(data_w7_parent, data_w7_CM, by = "MCSID")

#No NAs
w7_parent_CM <- w7_parent_CM %>% 
  drop_na(P_w7_SDQ_01:P_w7_SDQ_25, CM_w7_SDQ_01:CM_w7_SDQ_25)

#Recode item names
w7_parent_CM1 <- w7_parent_CM %>%
  select(MCSID, report.x, P_w7_SDQ_01:P_w7_SDQ_25, ethnicity.x, sex_final.x, ASD_final.x) %>%
...
w7_parent_CM2 <- w7_parent_CM %>%
  select(MCSID, report.y, CM_w7_SDQ_01:CM_w7_SDQ_25, ethnicity.y, sex_final.y, ASD_final.y) %>%
...
data_w7_final <- bind_rows(w7_parent_CM1, w7_parent_CM2)
...
#using either dataset is okay since each participant has both parent and teacher scores
```
...
data_w5_desc_p %>% 
  ...
  theme_classic()
ggsave("w5 sex and ASD.jpg")
```

### Sex and ASD chi-squared

``` {r}
results <- chisq.test(x = data_w5_desc_p %>% pull(sex_final), # 1st grouping variable
                      y = data_w5_desc_p %>% pull(ASD_final), # 2nd grouping variable
...
eff_size <- cramersV(x = data_w5_desc_p %>% pull(sex_final), 
                     y = data_w5_desc_p %>% pull(ASD_final), 
                     correct = T)
eff_size
#Phi φ = .12
```
...
#no graph and no chi squared analyses due to predominant white population. Report proportions.
```

### Overall SDQ (Parental vs Teacher Scores)
...
data_w5_final %>% 
  group_by(report) %>% 
  summarise(mean_diff = mean(difficulty), 
...
data_w5_final %>% 
  ...
  labs(x = "Total Difficulty Scores", y = "Number of Participants") + 
  facet_wrap(~report, scales = "free") + 
  theme_classic() #normal distribution with slight positive skew / right skew
ggsave("w5 SDQ difficulties hist.jpg")

#prosocial
data_w5_final %>% 
  ggplot(aes(x = prosocial)) + 
  geom_histogram(binwidth = 1) +
  labs(x = "Prosocial Scores", y = "Number of Participants") + 
  facet_wrap(~report, scales = "free") + 
  theme_classic() #negative skew / left skew
ggsave("w5 SDQ prosocial hist.jpg")
```

### Parental SDQ ASD diagnoses comparisons
...
t.test(data_w5_desc_p %>% filter(ASD_final == "ASD") %>% pull(difficulty), data_w5_desc_p %>% filter(ASD_final == "Non-ASD") %>% pull(difficulty), paired = F) %>% tidy()
cohensD(data_w5_desc_p %>% filter(ASD_final == "ASD") %>% pull(difficulty), data_w5_desc_p %>% filter(ASD_final == "Non-ASD") %>% pull(difficulty), method = "unequal")

#t(256.58) = 18.15, p<.001, [5.14, 6.39], d = 1.34 (very strong effect)

#ASD vs non-ASD differences in parental SDQ prosocial
t.test(data_w5_desc_p %>% filter(ASD_final == "ASD") %>% pull(prosocial), data_w5_desc_p %>% filter(ASD_final == "Non-ASD") %>% pull(prosocial), paired = F) %>% tidy()
cohensD(data_w5_desc_p %>% filter(ASD_final == "ASD") %>% pull(prosocial), data_w5_desc_p %>% filter(ASD_final == "Non-ASD") %>% pull(prosocial), method = "unequal")

#t(254.40) = -12.01, p<.001, [-2.02, -1.45], d = .92 (strong effect)

#graph
#difficulty
data_w5_desc_p %>% ggplot(aes(x = difficulty)) + 
  geom_histogram(binwidth = 1) + 
  facet_wrap(~ASD_final, scales = "free") + 
  labs(x = "Total Difficulty Scores", y = "Number of Participants") + 
  theme_classic() #slight positive skew / right skew for non-ASD. ASD group normal distribution
ggsave("w5 parent SDQ difficulty split by ASD.jpg")

#prosocial
data_w5_desc_p %>% ggplot(aes(x = prosocial)) + 
  geom_histogram(binwidth = 1) + 
  facet_wrap(~ASD_final, scales = "free") + 
  labs(x = "Prosocial Subscale Scores", y = "Number of Participants") + 
  theme_classic() #negative / left skew, but stronger for non-ASD, scores on average lower for ASD
ggsave("w5 parent SDQ prosocial split by ASD.jpg")
```
...
### Teacher SDQ ASD diagnoses comparisons
...
#ASD vs non-ASD differences in teacher SDQ scores
t.test(data_w5_desc_t %>% filter(ASD_final == "ASD") %>% pull(difficulty), data_w5_desc_t %>% filter(ASD_final == "Non-ASD") %>% pull(difficulty), paired = F) %>% tidy()
cohensD(data_w5_desc_t %>% filter(ASD_final == "ASD") %>% pull(difficulty), data_w5_desc_t %>% filter(ASD_final == "Non-ASD") %>% pull(difficulty), method = "unequal")

#t(256.96) = 11.99, p<.001, [2.41, 3.35], d = .88 (strong effect)

#ASD vs non-ASD differences in teacher SDQ prosocial
t.test(data_w5_desc_t %>% filter(ASD_final == "ASD") %>% pull(prosocial), data_w5_desc_t %>% filter(ASD_final == "Non-ASD") %>% pull(prosocial), paired = F) %>% tidy()
cohensD(data_w5_desc_t %>% filter(ASD_final == "ASD") %>% pull(prosocial), data_w5_desc_t %>% filter(ASD_final == "Non-ASD") %>% pull(prosocial), method = "unequal")

#t(259.08) = -12.01, p<.001, [-2.28, -1.64], d = .86 (strong effect)

#graph
#difficulty
data_w5_desc_t %>% ggplot(aes(x = difficulty)) + 
  geom_histogram(binwidth = 1) + 
  facet_wrap(~ASD_final, scales = "free") + 
  labs(x = "Total Difficulty Scores", y = "Number of Participants") + 
  theme_classic() #slight positive skew / right skew for non-ASD. ASD group normal distribution
...
#prosocial
data_w5_desc_t %>% ggplot(aes(x = prosocial)) + 
  geom_histogram(binwidth = 1) + 
  facet_wrap(~ASD_final, scales = "free") + 
  labs(x = "Prosocial Subscale Scores", y = "Number of Participants") + 
  theme_classic() # #negative / left skew, but stronger for non-ASD, scores on average lower for ASD
ggsave("w5 teacher SDQ prosocial split by ASD.jpg")
```
...
#using either dataset is okay since each participant has both parent and self-reported scores
```
...
data_w7_desc_p %>% 
  ...
  labs(x = "ASD Diagnosis", y = "Number of Participants", fill = "Sex") + 
  theme_classic()
ggsave("w7 sex and ASD.jpg")
```

### Sex and ASD chi-squared

``` {r}
results <- chisq.test(x = data_w7_desc_p %>% pull(sex_final), # 1st grouping variable
                      y = data_w7_desc_p %>% pull(ASD_final), # 2nd grouping variable
                      correct = T) %>% # df=1, continuity correction
  tidy()
...
eff_size <- cramersV(x = data_w7_desc_p %>% pull(sex_final), 
                     y = data_w7_desc_p %>% pull(ASD_final), 
                     correct = T)
eff_size
#Phi φ = .10
```
...
### Overall SDQ (Parental vs CM Scores)
...
data_w7_final %>% 
  ...
  facet_wrap(~report, scales = "free") + 
  theme_classic() #normal distribution with slight positive skew / right skew
ggsave("w7 SDQ difficulties hist.jpg")
...
data_w7_final %>% 
  ...
  facet_wrap(~report, scales = "free") + 
  theme_classic() #negative skew / left skew
ggsave("w7s SDQ prosocial hist.jpg")
```
...
#ASD vs non-ASD differences in parental SDQ scores
t.test(data_w7_desc_p %>% filter(ASD_final == "ASD") %>% pull(difficulty), data_w7_desc_p %>% filter(ASD_final == "Non-ASD") %>% pull(difficulty), paired = F) %>% tidy()
cohensD(data_w7_desc_p %>% filter(ASD_final == "ASD") %>% pull(difficulty), data_w7_desc_p %>% filter(ASD_final == "Non-ASD") %>% pull(difficulty), method = "unequal")

#t(240.96) = 13.63, p<.001, [3.80, 5.08], d = 1.03 (very strong effect)

#ASD vs non-ASD differences in parental SDQ prosocial
t.test(data_w7_desc_p %>% filter(ASD_final == "ASD") %>% pull(prosocial), data_w7_desc_p %>% filter(ASD_final == "Non-ASD") %>% pull(prosocial), paired = F) %>% tidy()
cohensD(data_w7_desc_p %>% filter(ASD_final == "ASD") %>% pull(prosocial), data_w7_desc_p %>% filter(ASD_final == "Non-ASD") %>% pull(prosocial), method = "unequal")

#t(240.79) = -11.42, p<.001, [-2.01, -1.42], d = .86 (strong effect)

#graph
#difficulty
data_w7_desc_p %>% ggplot(aes(x = difficulty)) + 
  geom_histogram(binwidth = 1) + 
  facet_wrap(~ASD_final, scales = "free") + 
  labs(x = "Total Difficulty Scores", y = "Number of Participants") + 
  theme_classic() #slight positive skew / right skew for non-ASD. ASD group normal distribution
...
#prosocial
data_w7_desc_p %>% ggplot(aes(x = prosocial)) + 
  geom_histogram(binwidth = 1) + 
  facet_wrap(~ASD_final, scales = "free") + 
  labs(x = "Prosocial Subscale Scores", y = "Number of Participants") + 
  theme_classic() #negative / left skew, but stronger for non-ASD, scores on average lower for ASD
ggsave("w7 parent SDQ prosocial split by ASD.jpg")
```
...
### Self-Reported SDQ ASD diagnoses comparisons
...
#ASD vs non-ASD differences in teacher SDQ scores
t.test(data_w7_desc_cm %>% filter(ASD_final == "ASD") %>% pull(difficulty), data_w7_desc_cm %>% filter(ASD_final == "Non-ASD") %>% pull(difficulty), paired = F) %>% tidy()
cohensD(data_w7_desc_cm %>% filter(ASD_final == "ASD") %>% pull(difficulty), data_w7_desc_cm %>% filter(ASD_final == "Non-ASD") %>% pull(difficulty), method = "unequal")

#t(242.79) = 6.92, p<.001, [1.71, 3.07], d = .51 (moderate effect)

#ASD vs non-ASD differences in teacher SDQ prosocial
t.test(data_w7_desc_cm %>% filter(ASD_final == "ASD") %>% pull(prosocial), data_w7_desc_cm %>% filter(ASD_final == "Non-ASD") %>% pull(prosocial), paired = F) %>% tidy()
cohensD(data_w7_desc_cm %>% filter(ASD_final == "ASD") %>% pull(prosocial), data_w7_desc_cm %>% filter(ASD_final == "Non-ASD") %>% pull(prosocial), method = "unequal")

#t(242.47) = -7.09, p<.001, [-1.30, -.73], d = .52 (moderate effect)

#graph
#difficulty
data_w7_desc_cm %>% ggplot(aes(x = difficulty)) + 
  geom_histogram(binwidth = 1) + 
  facet_wrap(~ASD_final, scales = "free") + 
  labs(x = "Total Difficulty Scores", y = "Number of Participants") + 
  theme_classic() #slight positive skew / right skew for non-ASD. ASD group normal distribution
ggsave("w7 teacher SDQ difficulty split by ASD.jpg")

#prosocial
data_w7_desc_cm %>% ggplot(aes(x = prosocial)) + 
  geom_histogram(binwidth = 1) + 
  facet_wrap(~ASD_final, scales = "free") + 
  labs(x = "Prosocial Subscale Scores", y = "Number of Participants") + 
  theme_classic() # #negative / left skew, but stronger for non-ASD
ggsave("w7 teacher SDQ prosocial split by ASD.jpg")
```
...
## Wave 5 and 7 (cross wave comparisons??)
...
#decided not to conduct any cross wave comparisons due to attrition bias
```
...
Need to create working dataset format

```{r}
w5_measurement_td <- data_w5_final %>%
  filter(ASD_final == "Non-ASD") %>% 
  select(-emotional, -conduct, -hyperactivity, -peer, -prosocial)
```
...
### CFA to validate 5-factor structure

``` {r}
#Parent report
w5_measurement_td_p <- w5_measurement_td %>% 
  filter(report == "parent")
parent_w5_td <- cfa(SDQ, data = w5_measurement_td_p, std.lv = TRUE, meanstructure = TRUE)
summary(parent_w5_td, rsquare = T, standardized = T, fit.measures = T)
#cfi .854 (Poor), rmsea .053(good), srmr, .042 (good)

#Teacher report
w5_measurement_td_t <- w5_measurement_td %>% 
  filter(report == "teacher")
teacher_w5_td <- cfa(SDQ, data = w5_measurement_td_t, std.lv = TRUE, meanstructure = TRUE)
summary(teacher_w5_td, rsquare = T, standardized = T, fit.measures = T)
#cfi 0.855 (poor), rmsea 0.071(fair fit), srmr, 0.060 (good)

rm(w5_measurement_td_p)
rm(w5_measurement_td_t)
```
...
``` {r}
#Plot
semPaths(object = parent_w5_td, #model object
...
#cfi 0.854 (poor), rmsea 0.062 (fair fit), srmr 0.051 (good)
```
...
```{r}
stru_inv_w5_td <- cfa(SDQ, data = w5_measurement_td, group = "report", group.equal = c("intercepts", "loadings", "residuals", "lv.variances", "lv.covariances"), std.lv=TRUE)
summary(stru_inv_w5_td, rsquare = T, fit.measures=TRUE, standardized=TRUE)
```

### Structural Invariance + Equal Factor Means

```{r}
strum_inv_w5_td <- cfa(SDQ, data = w5_measurement_td, group = "report", group.equal = c("intercepts", "loadings", "residuals",  "lv.variances", "lv.covariances", "means"), std.lv=TRUE)
summary(strum_inv_w5_td, rsquare = T, fit.measures=TRUE, standardized=TRUE)
```

### Comparisons

```{r}
comp_fit <- compareFit(con_inv_w5_td, met_inv_w5_td, sca_inv_w5_td, stri_inv_w5_td, stru_inv_w5_td, strum_inv_w5_td)
summary(comp_fit,  fit.measures = c("cfi", "rmsea", "srmr"))
```

``` {r}
rm(comp_fit)
rm(w5_measurement_td)
```
...
peer =~ Q6 + Q11 + Q14 + Q19 + Q23
prosocial =~ Q1 + Q4 + Q9 + Q17 + Q20
" ##same throughout all MI comparisons
```
...
``` {r}
#Parent report
w5_measurement_asd_p <- w5_measurement_asd %>% 
  filter(report == "parent")
parent_w5_asd <- cfa(SDQ, data = w5_measurement_asd_p, std.lv = TRUE, meanstructure = TRUE)
summary(parent_w5_asd, rsquare = T, standardized = T, fit.measures = T)
#cfi .831 (Poor), rmsea .071 (fair fit), srmr, .070 (good)

#Teacher report
w5_measurement_asd_t <- w5_measurement_asd %>% 
  filter(report == "teacher")
teacher_w5_asd <- cfa(SDQ, data = w5_measurement_asd_t, std.lv = TRUE, meanstructure = TRUE)
summary(teacher_w5_asd, rsquare = T, standardized = T, fit.measures = T)
#cfi .817 (poor), rmsea .079 (fair fit), srmr, .082 (poor)

rm(w5_measurement_asd_p)
rm(w5_measurement_asd_t)
```
...
#.824 (poor cfi), .075 (rmsea fair fit), .076 (good)
```
...
```{r}
stru_inv_w5_asd <- cfa(SDQ, data = w5_measurement_asd, group = "report", group.equal = c("intercepts", "loadings", "residuals", "lv.variances", "lv.covariances"), std.lv=TRUE)
summary(stru_inv_w5_asd, rsquare = T, fit.measures=TRUE, standardized=TRUE)
```

### Structural + Equal Factor Means

```{r}
strum_inv_w5_asd <- cfa(SDQ, data = w5_measurement_asd, group = "report", group.equal = c("intercepts", "loadings", "residuals", "lv.variances", "lv.covariances", "means"), std.lv=TRUE)
summary(strum_inv_w5_asd, rsquare = T, fit.measures=TRUE, standardized=TRUE)
```

### Comparisons

```{r}
comp_fit <- compareFit(con_inv_w5_asd, met_inv_w5_asd, sca_inv_w5_asd, stri_inv_w5_asd, stru_inv_w5_asd, strum_inv_w5_asd)
summary(comp_fit,  fit.measures = c("cfi", "rmsea", "srmr"))
```

``` {r}
rm(comp_fit)
rm(w5_measurement_asd)
```
...
``` {r}
#Parent report
w7_measurement_td_p <- w7_measurement_td %>% 
  filter(report == "parent")
parent_w7_td <- cfa(SDQ, data = w7_measurement_td_p, std.lv = TRUE, meanstructure = TRUE)
summary(parent_w7_td, rsquare = T, standardized = T, fit.measures = T)
#cfi .846 (Poor), rmsea .058 (good), srmr, .049 (good)

#self report
w7_measurement_td_cm <- w7_measurement_td %>% 
  filter(report == "self")
self_w7_td <- cfa(SDQ, data = w7_measurement_td_cm, std.lv = TRUE, meanstructure = TRUE)
summary(self_w7_td, rsquare = T, standardized = T, fit.measures = T)
#cfi .839 (poor), rmsea .052 (good), srmr, .049 (good)

rm(w7_measurement_td_p)
rm(w7_measurement_td_cm)
```
...
#cfi .843 (poor) , rmsea .055 (good), srmr .049 (good)
```
...
```{r}
stru_inv_w7_td <- cfa(SDQ, data = w7_measurement_td, group = "report", group.equal = c("intercepts", "loadings", "residuals", "lv.variances", "lv.covariances"), std.lv=TRUE)
...
```{r}
strum_inv_w7_td <- cfa(SDQ, data = w7_measurement_td, group = "report", group.equal = c("intercepts", "loadings", "residuals","lv.variances", "lv.covariances", "means"), std.lv=TRUE)
summary(strum_inv_w7_td, rsquare = T, fit.measures=TRUE, standardized=TRUE)
```

### Comparisons

```{r}
comp_fit <- compareFit(con_inv_w7_td, met_inv_w7_td, sca_inv_w7_td, stri_inv_w7_td, stru_inv_w7_td, strum_inv_w7_td)
summary(comp_fit,  fit.measures = c("cfi", "rmsea", "srmr"))
```

``` {r}
rm(comp_fit)
rm(w7_measurement_td)
```
...
``` {r}
#Parent report
w7_measurement_asd_p <- w7_measurement_asd %>% 
  filter(report == "parent")
parent_w7_asd <- cfa(SDQ, data = w7_measurement_asd_p, std.lv = TRUE, meanstructure = TRUE)
summary(parent_w7_asd, rsquare = T, standardized = T, fit.measures = T)
#cfi .862 (Poor), rmsea .067 (fair fit), srmr, .067 (good)

#self report
w7_measurement_asd_cm <- w7_measurement_asd %>% 
  filter(report == "self")
self_w7_asd <- cfa(SDQ, data = w7_measurement_asd_cm, std.lv = TRUE, meanstructure = TRUE)
summary(self_w7_asd, rsquare = T, standardized = T, fit.measures = T)
#cfi .739 (poor), rmsea .082 (poor), srmr, .097 (bad)

rm(w7_measurement_asd_p)
rm(w7_measurement_asd_cm)
```
...
#cfi .807 (poor), rmsea .075 (fair fit), .082 (poor)
```
...
```{r}
stru_inv_w7_asd <- cfa(SDQ, data = w7_measurement_asd, group = "report", group.equal = c("intercepts", "loadings", "residuals", "lv.variances", "lv.covariances"), std.lv=TRUE)
...
```{r}
strum_inv_w7_asd <- cfa(SDQ, data = w7_measurement_asd, group = "report", group.equal = c("intercepts", "loadings", "residuals", "lv.variances", "lv.covariances", "means"), std.lv=TRUE)
summary(strum_inv_w7_asd, rsquare = T, fit.measures=TRUE, standardized=TRUE)
```

### Comparisons

```{r}
comp_fit <- compareFit(con_inv_w7_asd, met_inv_w7_asd, sca_inv_w7_asd, stri_inv_w7_asd, stru_inv_w7_asd, strum_inv_w7_asd)
summary(comp_fit,  fit.measures = c("cfi", "rmsea", "srmr"))
```
